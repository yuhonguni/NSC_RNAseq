...
heatplot(KEGG_polg_ctrl_enrich('nsc_up'))
heatplot(KEGG_polg_ctrl_enrich('nsc_down'))
...
wiki_polg_ctrl_enrich('nsc_down')
...
polg_ctrl_nsc %>% filter(EnsemblID %in% MitoGenes_id)

polg_ctrl_ipsc %>% filter(EnsemblID %in% MitoGenes_id)
